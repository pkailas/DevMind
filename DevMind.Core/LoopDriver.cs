       ...
       public LoopDriver(
            ...
            CancellationToken ct)
        {
            ...
            if (lastToolCalls != null && lastToolCalls.Count > 0)
            {
                ...
                // No implicit DONE (H-20). A successful `dotnet run`/`dotnet exec` used to end
                // the turn when it was the first run of a pure-shell turn — meant for chat
                // models that never call task_done. It fired on research: job-1693 ran one
                // PowerShell script that wrote a probe Program.cs via Set-Content (invisible to
                // the mutation gate) and `dotnet run` it after ~20 read iterations, and the
                // job ended `done` with no answer and no work. A run is never evidence the task
                // is finished. The loop ends on task_done / ask_caller, the prose-finish
                // re-prompt, the depth cap, cancellation or the stop conditions below.
