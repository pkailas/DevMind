...
{
    // ── Serial execution (verified 2026-08-18) ──────────────────────────────
    //
    // Serial execution for this project comes from xunit.runner.json
    // (parallelizeTestCollections false, maxParallelThreads 1). xUnit v3 reads
    // and honours that file exactly as v2 did — verified 2026-08-18 by a
    // cross-class probe sharing a static in-flight counter across three test
    // classes: with parallelizeTestCollections=false / maxParallelThreads=1 the
    // peak cross-class concurrency was 1 (serial, ~2s); with
    // parallelizeTestCollections=true / maxParallelThreads=8 the peak was 3
    // (parallel, ~1s). The earlier contrary conclusion (commit 0c29203: "v3 does
    // NOT read xunit.runner.json; an in-test static lock is the ONLY mechanism
    // that serializes under v3") was wrong — its runner.json contained // comments
    // inside the JSON object (invalid JSON, so the runner failed to parse it) and
    // its canary counted entries in the test class CONSTRUCTOR, which xUnit never
    // parallelises (all 8 tests live in one class = one collection). An earlier
    // version of this file carried an in-test static lock (RunSerialized) purely to
    // work around that disproven claim; it was removed 2026-08-18. No lock is
    // needed here — this class's tests are side-effect-free reads of stock Editor
    // bindings.
